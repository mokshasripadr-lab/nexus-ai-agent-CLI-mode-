...
def _lock_down():
    """Make the config dir private to you (0700) and the file 0600."""
    try:
...
def load_config() -> dict:
    if CONFIG_FILE.exists():
        _lock_down()  # tighten perms every time, in case they were loosened
        try: return json.loads(CONFIG_FILE.read_text())
        except Exception: pass
    return {"provider": "google", "keys": []}


def redact(text: str, keys: list[str] | None = None) -> str:
    """Strip any API keys (and common key patterns) out of a string before it is
    ever printed — so a key can never leak through an error message or log."""
    if not text:
        return text
    for k in (keys or load_config().get("keys", [])):
        if k and len(k) > 6:
            text = text.replace(k, k[:4] + "…•redacted•")
    # Catch stray key-shaped tokens even if not in our list.
    import re as _re
    text = _re.sub(r"(AIza[0-9A-Za-z_\-]{10,}|sk-[A-Za-z0-9_\-]{10,})", "•redacted•", text)
    return text
...
def local_menu():
    """Pick which free local (Ollama) model Nexus falls back to — your Gen Wave model."""
    cfg = load_config()
    cur = cfg.get("local_model", "gemma2:2b")
    models = []
    try:
        ...
        pass
    if not models:
        print(f"{C.orange}No local models found.{C.r} Install Ollama (ollama.com), run "
              f"{C.clay}ollama pull gemma2:2b{C.r}, then try /local again.\n")
        return
    print(f"{C.cream}{C.b}Pick your free local model (used when the cloud model is busy):{C.r}")
    for i, m in enumerate(models, 1):
        mark = f"{C.orange}●{C.r}" if m == cur else f"{C.dim}○{C.r}"
        print(f"  {mark} {C.b}{i}{C.r}  {m}")
    choice = input(f"{C.orange}choose 1-{len(models)} (Enter = keep) ›{C.r} ").strip()
    if choice.isdigit() and 1 <= int(choice) <= len(models):
        cfg["local_model"] = models[int(choice) - 1]
        save_config(cfg)
    print(f"{C.green}→ local backup: {load_config().get('local_model', 'gemma2:2b')}{C.r}\n")
...
def _looks_failed(answer: str) -> bool:
    """Our error replies start with the red colour code; real answers don't."""
    return (not answer) or answer.startswith(C.red)


def _call_ollama(system, prompt, history) -> str | None:
    """Free local fallback via Ollama (http://localhost:11434). Uses no API
    credits and works offline. Returns None if Ollama isn't running."""
    model = load_config().get("local_model", "gemma2:2b")
    ...
    try:
        ...
        return None  # Ollama not installed / model not pulled / not running


def ask(prompt: str, history: list[dict], keys: list[str]) -> str:
    """Route to the chosen cloud model; if it fails, fall back to a FREE local
    model so you never get stuck or run out of API credits."""
    SYSTEM = ENGINES[current["key"]][3]
    cfg = load_config()
    provider = cfg.get("provider", "google")
    model = cfg.get("model") or MODEL
    if provider == "anthropic":
        answer = _call_anthropic(SYSTEM, prompt, history, keys, model)
    elif provider == "openai":
        answer = _call_openai(SYSTEM, prompt, history, keys, model)
    else:
        answer = _call_gemini(SYSTEM, prompt, history, keys, model)
    # Cloud model rate-limited or errored? Try the free local backend.
    if _looks_failed(answer):
        local = _call_ollama(SYSTEM, prompt, history)
        if local:
            return local.rstrip() + f"\n{C.dim}(answered by your free local model — no API credits used){C.r}"
    return answer


def _gemini_try(system, prompt, history, keys, model):
    """Try one Gemini model across all keys.
    Returns (text, status) — status is 'ok', 'ratelimited', 'nokey', or an error string."""
    contents = []
    for h in history[-8:]:
    ...
    # Key travels in a header, never the URL (keeps it out of logs/traces).
    ...
    for key in keys:
        ...
        try:
            ...
            if e.code in (429, 403):
                status = "ratelimited"; continue  # try the next key
            return None, f"Gemini error {e.code}. Check your key with /model."
        except Exception as e:
            return None, f"Error: {redact(str(e), keys)}"
    return None, status


def _call_gemini(system, prompt, history, keys, model):
    text, status = _gemini_try(system, prompt, history, keys, model)
    if text is not None:
        return text
    # Free-tier Pro is heavily rate-limited — fall back to Flash automatically so
    # the CLI keeps working instead of erroring out.
    if status == "ratelimited" and model != "gemini-2.5-flash":
        text, status = _gemini_try(system, prompt, history, keys, "gemini-2.5-flash")
        if text is not None:
            return text + f"\n{C.dim}(Pro was rate-limited — answered with Gemini Flash){C.r}"
    if status == "ratelimited":
        return (f"{C.red}Rate limit hit — wait ~1 min, or add a key from a DIFFERENT "
                f"Google account with /key (keys from the same account share one quota).{C.r}")
    if status == "nokey":
        return f"{C.red}No key found. Run /model to set one up.{C.r}"
    return f"{C.red}{status}{C.r}"
...
def _call_anthropic(system, prompt, history, keys, model):
    ...
    for ki, key in enumerate(keys):
        ...
        try:
            ...
            return f"{C.red}Error: {redact(str(e), keys)}{C.r}"
    return f"{C.red}No key found. Run /model to set one up.{C.r}"
...
def security_report():
    """Show what protects your key, and self-check file permissions."""
    print(f"{C.cream}{C.b}How your API key is protected{C.r}")
    print(f"  {C.clay}•{C.r} Stored only on THIS computer at ~/.nexus/config.json")
    print(f"  {C.clay}•{C.r} File locked to you only (chmod 600), folder 700")
    print(f"  {C.clay}•{C.r} Sent ONLY to the official model API over HTTPS — nowhere else")
    print(f"  {C.clay}•{C.r} Never placed in a URL, never printed, redacted from all errors")
    print(f"  {C.clay}•{C.r} Never bundled with the code or pushed to GitHub")
    ...
    try:
        ...
        if CONFIG_FILE.exists():
            ...
            if mode & 0o077:
                _lock_down(); print(f"  {C.orange}! tightened loose file permissions just now{C.r}"); ok = False
    except Exception:
        pass
    cfg = load_config()
    n = len(cfg.get("keys", []))
    print(f"\n  {C.grey}provider: {PROVIDERS_BY_ID.get(cfg.get('provider','google'),'?')} · {n} key(s) on file{C.r}")
    print(f"  {C.green}✓ config is locked down{C.r}\n" if ok else f"  {C.green}✓ fixed and locked down{C.r}\n")
    print(f"  {C.dim}Reality check: no local tool can stop someone who already has full")
    print(f"  access to your unlocked Mac. Keep your Mac password-protected, and if a")
    print(f"  key ever leaks, rotate it at the provider — that instantly kills the old one.{C.r}\n")
...
HELP = f"""{C.cream}{C.b}Commands{C.r}
  ...
  {C.clay}/local{C.r}   pick your free local (Ollama) fallback model
  {C.clay}/security{C.r} show how your key is protected + check file locks
